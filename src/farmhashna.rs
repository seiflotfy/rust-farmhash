...
pub fn na_hash64(mut s: &[u8]) -> u64 {
	...
	while i != end {
        x = rotate64(x.wrapping_add(y).wrapping_add(v.first).wrapping_add(fetch64(&s[8..])),
                37).wrapping_mul(K1);
    	y = rotate64(y.wrapping_add(v.second).wrapping_add(fetch64(&s[48..])),
                42).wrapping_mul(K1);
		x ^= w.second;
		y = y.wrapping_add(v.first).wrapping_add(fetch64(&s[40..]));
		z = rotate64(z.wrapping_add(w.first), 33).wrapping_mul(K1);
		v = farmhashcc_shared::weak_hash_len_32_with_seeds_bytes(&s,
                v.second.wrapping_mul(K1), x.wrapping_add(w.first));
		w = farmhashcc_shared::weak_hash_len_32_with_seeds_bytes(&s[32..],
                z.wrapping_add(w.second), y.wrapping_add(fetch64(&s[16..])));
        let t = z;
        z = x;
        x = t;
		s = &s[64..];
